...
from plotting import (
    plot_stage_one_duration,
    plot_stage_two_duration,
    plot_stage_three_duration,
    plot_stage_four_install_duration,
...
def main() -> None:
    ...
    if run_button:
        with st.spinner("Running simulation..."):
            ...
            with col1:
                fig1 = plot_stage_one_duration(df_manager.sim_df)
                st.pyplot(fig1)
            with col2:
                fig2 = plot_stage_two_duration(df_manager.sim_df)
                st.pyplot(fig2)
            
            col3, col4 = st.columns(2)
            with col3:
                fig3 = plot_stage_three_duration(df_manager.sim_df)
                st.pyplot(fig3)
            with col4:
                fig4 = plot_stage_four_install_duration(df_manager.sim_df)
                st.pyplot(fig4)

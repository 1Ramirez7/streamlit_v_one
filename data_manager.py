...
class DataFrameManager:
    ...
    - stage_four_available_df: Parts waiting in inventory
    ...
    def __init__(self, n_total_parts, n_total_aircraft, sim_time,
        ...
        self.stage_four_available_df = self._create_stage_four_available_df()
    ...
    def _create_stage_four_available_df(self):
        """
        ...
        stage_four_available_df <- if (length(leftover_parts) > 0) {
        ...
        if len(leftover_parts) > 0:
            ...
            return pd.DataFrame({
                ...
                'stage_one_duration': [np.nan] * len(leftover_parts),
                'stage_two_duration': [np.nan] * len(leftover_parts),
                'stage_three_duration': [np.nan] * len(leftover_parts),
                'stage_four_available_duration': [np.nan] * len(leftover_parts),
                'stage_four_install_duration': [np.nan] * len(leftover_parts),
                'stage_one_start': [np.nan] * len(leftover_parts),
                'stage_one_end': [np.nan] * len(leftover_parts),
                'stage_two_start': [np.nan] * len(leftover_parts),
                'stage_two_end': [np.nan] * len(leftover_parts),
                'stage_three_start': [np.nan] * len(leftover_parts),
                'stage_three_end': [np.nan] * len(leftover_parts),
                'destwo_id': [None] * len(leftover_parts),
                'actwo_id': [None] * len(leftover_parts),
                'stage_four_available_start': [1.0] * len(leftover_parts),
                'stage_four_available_end': [np.nan] * len(leftover_parts),
                'stage_four_install_start': [np.nan] * len(leftover_parts),
                'stage_four_install_end': [np.nan] * len(leftover_parts),
            ...
            return pd.DataFrame({
                ...
                'stage_one_duration': pd.Series(dtype='float64'),
                'stage_two_duration': pd.Series(dtype='float64'),
                'stage_three_duration': pd.Series(dtype='float64'),
                'stage_four_available_duration': pd.Series(dtype='float64'),
                'stage_four_install_duration': pd.Series(dtype='float64'),
                'stage_one_start': pd.Series(dtype='float64'),
                'stage_one_end': pd.Series(dtype='float64'),
                'stage_two_start': pd.Series(dtype='float64'),
                'stage_two_end': pd.Series(dtype='float64'),
                'stage_three_start': pd.Series(dtype='float64'),
                'stage_three_end': pd.Series(dtype='float64'),
                'destwo_id': pd.Series(dtype='Int64'),
                'actwo_id': pd.Series(dtype='Int64'),
                'stage_four_available_start': pd.Series(dtype='float64'),
                'stage_four_available_end': pd.Series(dtype='float64'),
                'stage_four_install_start': pd.Series(dtype='float64'),
                'stage_four_install_end': pd.Series(dtype='float64'),
    ...
    def _create_sim_df(self):
        ...
        return pd.DataFrame({
            ...
            'stage_one_duration': [np.nan] * self.max_sim_events,
            'stage_two_duration': [np.nan] * self.max_sim_events,
            'stage_three_duration': [np.nan] * self.max_sim_events,
            'stage_four_available_duration': [np.nan] * self.max_sim_events,
            'stage_four_install_duration': [np.nan] * self.max_sim_events,
            'stage_one_start': [np.nan] * self.max_sim_events,
            'stage_one_end': [np.nan] * self.max_sim_events,
            'stage_two_start': [np.nan] * self.max_sim_events,
            'stage_two_end': [np.nan] * self.max_sim_events,
            'stage_three_start': [np.nan] * self.max_sim_events,
            'stage_three_end': [np.nan] * self.max_sim_events,
            'destwo_id': [None] * self.max_sim_events,
            'actwo_id': [None] * self.max_sim_events,
            'stage_four_available_start': [np.nan] * self.max_sim_events,
            'stage_four_available_end': [np.nan] * self.max_sim_events,
            'stage_four_install_start': [np.nan] * self.max_sim_events,
            'stage_four_install_end': [np.nan] * self.max_sim_events,
    ...
    def _create_des_df(self):
        ...
        return pd.DataFrame({
            ...
            'stage_one_duration': [np.nan] * self.max_des_events,
            'stage_one_start': [np.nan] * self.max_des_events,
            'stage_one_end': [np.nan] * self.max_des_events,
            ...
            'stage_four_install_duration': [np.nan] * self.max_des_events,
            'stage_four_install_start': [np.nan] * self.max_des_events,
            'stage_four_install_end': [np.nan] * self.max_des_events
        })

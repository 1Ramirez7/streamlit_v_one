...
class SimulationEngine:
    ...
    def calculate_stage_one_duration(self):
        """
    ...
    def calculate_stage_two_duration(self):
        """
    ...
    def calculate_stage_three_duration(self):
        """
    ...
    def calculate_stage_four_install_duration(self):
        """
    ...
    def add_sim_event(self, sim_id, part_id, desone_id, acone_id, micap,
                      stage_one_duration, stage_two_duration, stage_three_duration,
                      stage_four_available_duration, stage_four_install_duration,
                      stage_one_start, stage_one_end,
                      stage_two_start, stage_two_end,
                      stage_three_start, stage_three_end,
                      destwo_id, actwo_id,
                      stage_four_available_start, stage_four_available_end,
                      stage_four_install_start, stage_four_install_end,
                      cycle, condemn):
        """
        ...
        self.df.sim_df.at[row_idx, 'stage_one_duration'] = stage_one_duration
        self.df.sim_df.at[row_idx, 'stage_two_duration'] = stage_two_duration
        self.df.sim_df.at[row_idx, 'stage_three_duration'] = stage_three_duration
        self.df.sim_df.at[row_idx, 'stage_four_available_duration'] = stage_four_available_duration
        self.df.sim_df.at[row_idx, 'stage_four_install_duration'] = stage_four_install_duration
        self.df.sim_df.at[row_idx, 'stage_one_start'] = stage_one_start
        self.df.sim_df.at[row_idx, 'stage_one_end'] = stage_one_end
        self.df.sim_df.at[row_idx, 'stage_two_start'] = stage_two_start
        self.df.sim_df.at[row_idx, 'stage_two_end'] = stage_two_end
        self.df.sim_df.at[row_idx, 'stage_three_start'] = stage_three_start
        self.df.sim_df.at[row_idx, 'stage_three_end'] = stage_three_end
        self.df.sim_df.at[row_idx, 'destwo_id'] = destwo_id
        self.df.sim_df.at[row_idx, 'actwo_id'] = actwo_id
        self.df.sim_df.at[row_idx, 'stage_four_available_start'] = stage_four_available_start
        self.df.sim_df.at[row_idx, 'stage_four_available_end'] = stage_four_available_end
        self.df.sim_df.at[row_idx, 'stage_four_install_start'] = stage_four_install_start
        self.df.sim_df.at[row_idx, 'stage_four_install_end'] = stage_four_install_end
    ...
    def add_des_event(self, des_id, ac_id, micap,
                      ...
                      stage_one_duration, stage_one_start, stage_one_end,
                      micap_duration, micap_start, micap_end,
                      simtwo_id, parttwo_id,
                      stage_four_install_duration,
                      stage_four_install_start,
                      stage_four_install_end):
        """
        ...
        add_des_event <- function(df, row_idx,
          ...
          ...
          df$stage_four_install_end[row_idx] <- stage_four_install_end
        ...
        self.df.des_df.at[row_idx, 'stage_one_duration'] = stage_one_duration
        self.df.des_df.at[row_idx, 'stage_one_start'] = stage_one_start
        self.df.des_df.at[row_idx, 'stage_one_end'] = stage_one_end
        ...
        self.df.des_df.at[row_idx, 'stage_four_install_duration'] = stage_four_install_duration
        self.df.des_df.at[row_idx, 'stage_four_install_start'] = stage_four_install_start
        self.df.des_df.at[row_idx, 'stage_four_install_end'] = stage_four_install_end
    ...
    def process_new_cycle_stages(self, part_id, ac_id, s4_install_end, 
        ...
        process_new_cycle_stages <- function(sim_df, des_df, part_id, ac_id, 
          ...
          if (s1_end > sim_time) {
            ...
            sim_df$stage_one_duration[sim_row_idx] <- d1
            sim_df$stage_one_end[sim_row_idx] <- NA_real_
            
            # Update des_df - only stage 1 partial info
            des_df$stage_one_duration[des_row_idx] <- d1
            des_df$stage_one_end[des_row_idx] <- NA_real_
          ...
          sim_df$stage_one_duration[sim_row_idx] <- d1
          sim_df$stage_one_end[sim_row_idx] <- s1_end
          
          des_df$stage_one_duration[des_row_idx] <- d1
          des_df$stage_one_end[des_row_idx] <- s1_end
          ...
          if (s2_end > sim_time) {
            sim_df$stage_two_duration[sim_row_idx] <- d2
            sim_df$stage_two_start[sim_row_idx] <- s2_start
            sim_df$stage_two_end[sim_row_idx] <- NA_real_
          ...
          sim_df$stage_two_duration[sim_row_idx] <- d2
          sim_df$stage_two_start[sim_row_idx] <- s2_start
          sim_df$stage_two_end[sim_row_idx] <- s2_end
          ...
          if (s3_end > sim_time) {
            sim_df$stage_three_duration[sim_row_idx] <- d3
            sim_df$stage_three_start[sim_row_idx] <- s3_start
            sim_df$stage_three_end[sim_row_idx] <- NA_real_
          ...
          sim_df$stage_three_duration[sim_row_idx] <- d3
          sim_df$stage_three_start[sim_row_idx] <- s3_start
          sim_df$stage_three_end[sim_row_idx] <- s3_end
        ...
        # --- Stage One Calculation (for both part and aircraft) ---
        d1 = self.calculate_stage_one_duration()
        s1_start = s4_install_end
        s1_end = s1_start + d1
        ...
        if s1_end > self.sim_time:
            # Update sim_df - only stage 1 partial info
            self.df.sim_df.at[sim_row_idx, 'stage_one_duration'] = d1
            self.df.sim_df.at[sim_row_idx, 'stage_one_end'] = np.nan
            
            # Update des_df - only stage 1 partial info
            self.df.des_df.at[des_row_idx, 'stage_one_duration'] = d1
            self.df.des_df.at[des_row_idx, 'stage_one_end'] = np.nan
            
            return  # Early exit - simulation time exceeded
        
        # Stage 1 fits within sim_time, update both dataframes
        self.df.sim_df.at[sim_row_idx, 'stage_one_duration'] = d1
        self.df.sim_df.at[sim_row_idx, 'stage_one_end'] = s1_end
        
        self.df.des_df.at[des_row_idx, 'stage_one_duration'] = d1
        self.df.des_df.at[des_row_idx, 'stage_one_end'] = s1_end
        
        # --- Stage Two Calculation (part only) ---
        d2 = self.calculate_stage_two_duration()
        s2_start = s1_end
        s2_end = s2_start + d2
        
        # Check sim_time boundary for stage 2
        if s2_end > self.sim_time:
            self.df.sim_df.at[sim_row_idx, 'stage_two_duration'] = d2
            self.df.sim_df.at[sim_row_idx, 'stage_two_start'] = s2_start
            self.df.sim_df.at[sim_row_idx, 'stage_two_end'] = np.nan
        ...
        self.df.sim_df.at[sim_row_idx, 'stage_two_duration'] = d2
        self.df.sim_df.at[sim_row_idx, 'stage_two_start'] = s2_start
        self.df.sim_df.at[sim_row_idx, 'stage_two_end'] = s2_end
        
        # --- Stage Three Calculation (part only) ---
        d3 = self.calculate_stage_three_duration()
        s3_start = s2_end
        s3_end = s3_start + d3
        
        # Check sim_time boundary for stage 3
        if s3_end > self.sim_time:
            self.df.sim_df.at[sim_row_idx, 'stage_three_duration'] = d3
            self.df.sim_df.at[sim_row_idx, 'stage_three_start'] = s3_start
            self.df.sim_df.at[sim_row_idx, 'stage_three_end'] = np.nan
        ...
        self.df.sim_df.at[sim_row_idx, 'stage_three_duration'] = d3
        self.df.sim_df.at[sim_row_idx, 'stage_three_start'] = s3_start
        self.df.sim_df.at[sim_row_idx, 'stage_three_end'] = s3_end
    ...
    def initialize_first_cycle(self):
        ...
        for (row_idx in seq_len(nrow(aircraft_df))) {
          ...
          sim_df <- add_sim_event(
            ...
            micap = ac_row$micap,
            stage_one_duration = d1,
            stage_two_duration = NA_real_,
            stage_three_duration = NA_real_,
            stage_four_available_duration = NA_real_,
            stage_four_install_duration = NA_real_,
            stage_one_start = s1_start,
            stage_one_end = s1_end,
            stage_two_start = NA_real_,
            stage_two_end = NA_real_,
            stage_three_start = NA_real_,
            stage_three_end = NA_real_,
            destwo_id = NA_integer_,
            actwo_id = NA_integer_,
            stage_four_available_start = NA_real_,
            stage_four_available_end = NA_real_,
            stage_four_install_start = NA_real_,
            stage_four_install_end = NA_real_,
            cycle = 1L,
            condemn = "no"
          )
        ...
        for row_idx in range(len(self.df.aircraft_df)):
            ...
            self.add_sim_event(
                ...
                micap=ac_row['micap'],
                stage_one_duration=d1,
                stage_two_duration=np.nan,
                stage_three_duration=np.nan,
                stage_four_available_duration=np.nan,
                stage_four_install_duration=np.nan,
                stage_one_start=s1_start,
                stage_one_end=s1_end,
                stage_two_start=np.nan,
                stage_two_end=np.nan,
                stage_three_start=np.nan,
                stage_three_end=np.nan,
                destwo_id=None,
                actwo_id=None,
                stage_four_available_start=np.nan,
                stage_four_available_end=np.nan,
                stage_four_install_start=np.nan,
                stage_four_install_end=np.nan,
                cycle=1,
                condemn="no"
            )
    ...
    def initialize_stages_two_three(self):
        ...
        for (i in aircraft_df$part_id) {
          ...
          # --- Stage Two ---
          d2 <- max(0, rnorm(1, mean = stwo_mean, sd = stwo_sd))
          s2_start <- sim_df$stage_one_end[part_record_idx]
          s2_end   <- s2_start + d2
          ...
          sim_df$stage_two_duration[part_record_idx] <- d2
          sim_df$stage_two_start[part_record_idx] <- s2_start
          sim_df$stage_two_end[part_record_idx] <- s2_end
          sim_df$stage_three_duration[part_record_idx] <- d3
          sim_df$stage_three_start[part_record_idx] <- s3_start
          sim_df$stage_three_end[part_record_idx] <- s3_end
        }
        ...
        for part_id in self.df.aircraft_df['part_id']:
            ...
            # --- Stage Two ---
            d2 = self.calculate_stage_two_duration()
            s2_start = self.df.sim_df.at[part_record_idx, 'stage_one_end']
            s2_end = s2_start + d2
            ...
            self.df.sim_df.at[part_record_idx, 'stage_two_duration'] = d2
            self.df.sim_df.at[part_record_idx, 'stage_two_start'] = s2_start
            self.df.sim_df.at[part_record_idx, 'stage_two_end'] = s2_end
            self.df.sim_df.at[part_record_idx, 'stage_three_duration'] = d3
            self.df.sim_df.at[part_record_idx, 'stage_three_start'] = s3_start
            self.df.sim_df.at[part_record_idx, 'stage_three_end'] = s3_end
    ...
    def build_event_index(self, period):
        ...
        aircraft_index <- if (nrow(matching_aircraft) > 0) {
          matching_aircraft |>
            dplyr::select(des_id, ac_id, time = stage_one_end)
        } else {
        ...
        if len(matching_aircraft) > 0:
            ...
            aircraft_index['time'] = matching_aircraft['stage_one_end']
        else:
    ...
    def handle_part_completes_stage_three(self, sim_id):
        ...
        s3_end <- part_row$stage_three_end
        ...
        if (n_micap == 0) {
          # Add to stage_four_available_df
          stage_four_available_df <- stage_four_available_df |>
            dplyr::add_row(...)
        ...
        # CASE A1: No MICAP aircraft → Part goes to available inventory
        if n_micap == 0:
            # Create new row for stage_four_available_df
            new_row = pd.DataFrame([{
                ...
                'stage_one_duration': part_row['stage_one_duration'],
                'stage_two_duration': part_row['stage_two_duration'],
                'stage_three_duration': part_row['stage_three_duration'],
                'stage_four_available_duration': np.nan,
                'stage_four_install_duration': np.nan,
                'stage_one_start': part_row['stage_one_start'],
                'stage_one_end': part_row['stage_one_end'],
                'stage_two_start': part_row['stage_two_start'],
                'stage_two_end': part_row['stage_two_end'],
                'stage_three_start': part_row['stage_three_start'],
                'stage_three_end': part_row['stage_three_end'],
                'destwo_id': None,
                'actwo_id': None,
                'stage_four_available_start': s3_end,
                'stage_four_available_end': np.nan,
                'stage_four_install_start': np.nan,
                'stage_four_install_end': np.nan,
                'cycle': 1,
            ...
            self.df.stage_four_available_df = pd.concat(
                [self.df.stage_four_available_df, new_row], 
                ignore_index=True
            )
        ...
        # CASE A2: MICAP aircraft exists → Install part directly
            ...
            d4_install = self.calculate_stage_four_install_duration()
            s4_install_start = s3_end
            s4_install_end = s4_install_start + d4_install
            ...
            # Update existing sim_df row for this part (add stage 4 install info)
            part_row_idx = filled_sim_df[filled_sim_df['sim_id'] == sim_id].index[0]
            self.df.sim_df.at[part_row_idx, 'stage_four_available_duration'] = np.nan
            self.df.sim_df.at[part_row_idx, 'stage_four_install_duration'] = d4_install
            self.df.sim_df.at[part_row_idx, 'destwo_id'] = first_micap['des_id']
            self.df.sim_df.at[part_row_idx, 'actwo_id'] = first_micap['ac_id']
            self.df.sim_df.at[part_row_idx, 'stage_four_available_start'] = np.nan
            self.df.sim_df.at[part_row_idx, 'stage_four_available_end'] = np.nan
            self.df.sim_df.at[part_row_idx, 'stage_four_install_start'] = s4_install_start
            self.df.sim_df.at[part_row_idx, 'stage_four_install_end'] = s4_install_end
            ...
            self.add_sim_event(
                ...
                micap="no",
                stage_one_duration=np.nan,
                stage_two_duration=np.nan,
                stage_three_duration=np.nan,
                stage_four_available_duration=np.nan,
                stage_four_install_duration=np.nan,
                stage_one_start=s4_install_end,
                stage_one_end=np.nan,
                stage_two_start=np.nan,
                stage_two_end=np.nan,
                stage_three_start=np.nan,
                stage_three_end=np.nan,
            ...
            self.df.des_df.at[micap_des_row_idx, 'stage_four_install_duration'] = d4_install
            self.df.des_df.at[micap_des_row_idx, 'stage_four_install_start'] = s4_install_start
            self.df.des_df.at[micap_des_row_idx, 'stage_four_install_end'] = s4_install_end
    ...
    def handle_aircraft_needs_part(self, des_id):
        ...
        s1_end <- ac_row$stage_one_end
        old_part_id <- ac_row$partone_id
        
        # CHECK: Are there available parts?
        available_parts <- stage_four_available_df |>
        ...
        if (n_available > 0) {
          first_available <- available_parts |> 
            dplyr::arrange(stage_four_available_start, part_id) |>
            dplyr::slice(1)
          ...
          stage_four_available_end <- s4_install_start
          stage_four_available_duration <- stage_four_available_end - first_available$stage_four_available_start
        ...
        available_parts = self.df.stage_four_available_df[
            self.df.stage_four_available_df['part_id'].notna()
        ].copy()
        ...
        # CASE B1: Part Available
        if n_available > 0:
            # Get first available part (earliest stage_four_available_start, then lowest part_id)
            first_available = available_parts.sort_values(
                ['stage_four_available_start', 'part_id']
            ).iloc[0]
            ...
            stage_four_available_end = s4_install_start
            stage_four_available_duration = (
                stage_four_available_end - first_available['stage_four_available_start']
            )
            ...
            if has_sim_id:
                ...
                self.df.sim_df.at[part_row_idx, 'stage_four_available_duration'] = stage_four_available_duration
                self.df.sim_df.at[part_row_idx, 'stage_four_install_duration'] = d4_install
                self.df.sim_df.at[part_row_idx, 'destwo_id'] = des_id
                self.df.sim_df.at[part_row_idx, 'actwo_id'] = ac_row['ac_id']
                self.df.sim_df.at[part_row_idx, 'stage_four_available_start'] = first_available['stage_four_available_start']
                self.df.sim_df.at[part_row_idx, 'stage_four_available_end'] = stage_four_available_end
                self.df.sim_df.at[part_row_idx, 'stage_four_install_start'] = s4_install_start
                self.df.sim_df.at[part_row_idx, 'stage_four_install_end'] = s4_install_end
                ...
                self.df.des_df.at[ac_row_idx, 'stage_four_install_duration'] = d4_install
                self.df.des_df.at[ac_row_idx, 'stage_four_install_start'] = s4_install_start
                self.df.des_df.at[ac_row_idx, 'stage_four_install_end'] = s4_install_end
                ...
                self.add_sim_event(
                    ...
                    stage_two_duration=np.nan,
                    stage_three_duration=np.nan,
                    stage_four_available_duration=stage_four_available_duration,
                    stage_four_install_duration=d4_install,
                    stage_one_start=np.nan,
                    stage_one_end=np.nan,
                    stage_two_start=np.nan,
                    stage_two_end=np.nan,
                    stage_three_start=np.nan,
                    ...
                    stage_four_available_start=first_available['stage_four_available_start'],
                    stage_four_available_end=stage_four_available_end,
                    stage_four_install_start=s4_install_start,
                    stage_four_install_end=s4_install_end,
                    cycle=0,
                    condemn="no"
                )
            ...
            # Remove allocated part from available inventory
            self.df.stage_four_available_df = self.df.stage_four_available_df[
                self.df.stage_four_available_df['part_id'] != first_available['part_id']
            ].reset_index(drop=True)
        ...
        # CASE B2: No Parts Available → Aircraft Goes MICAP
            ...
            new_micap = pd.DataFrame([{
                ...
                'ac_id': ac_row['ac_id'],
                'micap': 'yes',
                'stage_one_duration': ac_row['stage_one_duration'],
                'stage_one_start': ac_row['stage_one_start'],
                'stage_one_end': ac_row['stage_one_end'],
                'micap_duration': np.nan,
